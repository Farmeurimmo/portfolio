...
<section class="items-center flex flex-col min-h-screen gap-3 border-top">
	...
	<div class="p-6 justify-center items-start grid md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-10">
		{#each posts as post}
			<a href="/blog/{post.id}" class="flex flex-col items-start p-5 bg-gray-900 rounded-2xl
     transform transition duration-500 hover:scale-105 hover:bg-gray-800 min-h-full" title={post.title}>
